...
class ParamActor_Threshold(nn.Module):
    ...
    def forward(self, state):
        ...
        # append threshold to the parameters
        # action_params = torch.cat((action_params, self.threshold))
        params_list = []
...
class PDQNAgent_Threshold(Agent):
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            if self.actor_param.stochastic:
                all_action_parameters, dist = self.actor_param.forward(state)
            else:
    ...
    def _optimize_td_loss(self):
        ...
        # ---------------------- optimize Q-network ----------------------
        #if self.learn_actor or self._episode > 1000:
        if self.learn_actor:
            # We only have one Q net, live with it!
            # suppose somehow we have the action dist
            with torch.no_grad():
                if self.actor_param_target.stochastic:
                    alpha = 1
                    _, pred_next_dist = self.actor_param_target.forward(next_states)
                    new_next_actions = pred_next_dist.rsample()
                    new_log_pi = pred_next_dist.log_prob(new_next_actions)
                    new_next_actions = new_next_actions.squeeze()
                    new_log_pi = new_log_pi.squeeze()
                    pred_Q_a =self.actor_target(next_states, new_next_actions)
                    Qprime = torch.max(pred_Q_a, 1, keepdim=True)[0].squeeze() - alpha * torch.sum(new_log_pi,dim=1)
        ...
        with torch.no_grad():
            if self.actor_param.stochastic:
                _, dist = self.actor_param_target.forward(next_states)
                action_params = pred_next_dist.rsample()
                log_pi = pred_next_dist.log_prob(action_params)
                action_params = action_params.squeeze()
        ...
        if self.indexed:
            ...
            if self.actor_param.stochastic:
                Q_loss = torch.mean(torch.sum(Q_val, 1) - alpha * torch.sum(log_pi,dim=1))
            else:
        ...
        if self.actor_param.stochastic:
            ...
            action_params,_ = self.actor_param(Variable(states))
        else:
    ...
    def retrain_param_with_Q(self):
        ...
        with torch.no_grad():
            if self.actor_param.stochastic:
                action_params,_ = self.actor_param(states)
            else:
                action_params = self.actor_param(states)
        action_params.requires_grad = True
        assert (self.weighted ^ self.average ^ self.random_weighted) or \
...
class MultiPassPDQNAgent_Baseline(PDQNAgent_Threshold):
