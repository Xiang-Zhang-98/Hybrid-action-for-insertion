...
class ParamActor_Threshold(nn.Module):
    # One thing to note is that the very last element for assembly primitives is the force limit.
    # In our work, we make is a parameter instead of a part of NN output. Thus, in order to use
    # Twin-smoothed MP-DQN for the general case, self.thresholds should be discarded, the output
    # size of NN should be self.action_parameter_size, and no nned to apend threshold during the forward
    def __init__(self, state_size, action_size, action_parameter_size, hidden_layers, squashing_function=False,
                 output_layer_init_std=None, init_type="kaiming", activation="relu", init_std=None, init_threshold=1.8):
        super(ParamActor_Threshold, self).__init__()
        ...
        if hidden_layers is not None:
            ...
            for i in range(1, nh):
                self.layers.append(nn.Linear(hidden_layers[i - 1], hidden_layers[i]))
            lastHiddenLayerSize = hidden_layers[nh - 1]
        # self.action_parameter_size-1 since there is one parameter for threshold and is state-independent
        self.action_parameters_output_layer = nn.Linear(lastHiddenLayerSize, self.action_parameter_size-self.action_size)
        self.action_parameters_passthrough_layer = nn.Linear(self.state_size, self.action_parameter_size-self.action_size)
        self.thresholds = []
        for i in range(self.action_size):
            self.thresholds.append(nn.Parameter(torch.tensor([init_threshold], dtype=torch.float32)))
        ...
        self.action_parameter_sizes = [4, 4, 1]
    ...
    def forward(self, state):
        ...
        for i in range(self.action_size):
            if i == 0:
                action_params_w_threshold = torch.cat((action_params[:, 0:(self.action_parameter_sizes[i]-1)],
                                                       self.thresholds[i].expand([action_params.shape[0], 1])), dim=1)
                offset = self.action_parameter_sizes[0]-1
            else:
                action_params_w_threshold = torch.cat((action_params_w_threshold,
                                       action_params[:, offset:(offset+self.action_parameter_sizes[i]-1)],
                                                       self.thresholds[i].expand([action_params.shape[0], 1])),
                                      dim=1)
                offset = offset + self.action_parameter_sizes[i]-1
        action_params = action_params_w_threshold
        action_params = action_params.squeeze()
        return action_params


class PDQNAgent_Threshold(Agent):
    """
    DDPG actor-critic agent for parameterised action spaces
    [Hausknecht and Stone 2016]
    """

    NAME = "P-DQN Agent Threshold"
    ...
    def __init__(self,
                 ...
                 actor_class=QActor,
                 actor_kwargs={},
                 actor_param_class=ParamActor_Threshold,
                 actor_param_kwargs={},
                 epsilon_initial=1.0,
                 epsilon_final=0.05,
                 epsilon_steps=10000,
                 ...
                 policy_noise=0.1,
                 noise_decay=False,
                 noise_decay_episode=10000):
        super(PDQNAgent_Threshold, self).__init__(observation_space, action_space)
        ...
        self.actor_twin = actor_class(self.observation_space.shape[0], self.num_actions, self.action_parameter_size,
                                 **actor_kwargs).to(device)
        self.actor_twin_target = actor_class(self.observation_space.shape[0], self.num_actions, self.action_parameter_size,
                                        **actor_kwargs).to(device)
        hard_update_target_network(self.actor_twin, self.actor_twin_target)
        self.actor_twin_target.eval()
        ...
        self.actor_twin_optimiser = optim.Adam(self.actor_twin.parameters(),
        ...
        # Smooth action for TD3
        self.policy_noise = policy_noise
        self.noise_clip = np.exp(-2)
        self.policy_freq = 1
        self.noise_decay = noise_decay
        self.noise_decay_episode = noise_decay_episode
        self.policy_noise_init = self.policy_noise
        print("params:")
        print(self.policy_noise)
        print(self.noise_decay)
        print(self.noise_decay_episode)
    ...
    def end_episode(self):
        ...
        if self.noise_decay:
            self.policy_noise = self.policy_noise_init - self._episode / self.noise_decay_episode * self.policy_noise_init
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            # Should we use random action during the rollout?
            parameters_noise = (torch.rand_like(all_action_parameters)*self.policy_noise).clamp(-self.noise_clip, self.noise_clip)
            all_action_parameters = torch.max(torch.min(all_action_parameters + parameters_noise,
                                                              self.action_parameter_max), self.action_parameter_min)
    ...
    def _optimize_td_loss(self):
        ...
        # ---------------------- optimize Q-network ----------------------
        if self.learn_actor:
            # suppose somehow we have the action dist
            with torch.no_grad():
                pred_next_action_parameters = self.actor_param_target.forward(next_states)
                parameters_noise = (torch.rand_like(pred_next_action_parameters)*self.policy_noise
                                    ).clamp(-self.noise_clip,self.noise_clip)
                pred_next_action_parameters = torch.max(torch.min(pred_next_action_parameters + parameters_noise,
                                                                  self.action_parameter_max), self.action_parameter_min)
                ...
                pred_Q_a_twin = self.actor_twin_target(next_states, pred_next_action_parameters)
                Qprime_twin = torch.max(pred_Q_a_twin, 1, keepdim=True)[0].squeeze()

                Qprime_clip = torch.min(Qprime, Qprime_twin)
                # Compute the TD error
                target = rewards + (1 - terminals) * self.gamma * Qprime_clip
            ...
            # Twin_Q net update
            q_values_twin = self.actor_twin(states, action_parameters)
            y_predicted_twin = q_values_twin.gather(1, actions.view(-1, 1)).squeeze()
            y_expected = target
            loss_Q_twin = self.loss_func(y_predicted_twin, y_expected)
            self.actor_twin_optimiser.zero_grad()
            loss_Q_twin.backward()
            if self.clip_grad > 0:
                torch.nn.utils.clip_grad_norm_(self.actor_twin.parameters(), self.clip_grad)
            self.actor_twin_optimiser.step()
        ...
        if self._step % self.policy_freq == 0:
            ...
            soft_update_target_network(self.actor_twin, self.actor_twin_target, self.tau_actor)
    ...
    def save_models(self, prefix):
        ...
        torch.save(self.actor_twin.state_dict(), prefix + '_actor_twin.pt')
    ...
    def load_models(self, prefix):
        ...
        self.actor_twin.load_state_dict(torch.load(prefix + '_actor_twin.pt', map_location='cpu'))
        self.actor_param.load_state_dict(torch.load(prefix + '_actor_param.pt', map_location='cpu'))
        print('Models loaded successfully')

    def load_Q(self, prefix):
        """
        loads the target actor and critic models, and copies them onto actor and critic models
        :param prefix: the count of episodes iterated (used to find the file name)
        :param target: whether to load the target newtwork too (not necessary for evaluation)
    ...
    def transfer_step(self, state, action, reward, next_state, next_action, terminal, time_steps=1):
        act, all_action_parameters = action
        self._step += 1

        self._add_sample(state, np.concatenate(([act],all_action_parameters)).ravel(), reward, next_state, np.concatenate(([next_action[0]],next_action[1])).ravel(), terminal=terminal)
        if self._step >= self.batch_size and self._step >= self.initial_memory_threshold:
            self.retrain_param_with_Q()
            self.updates += 1
...
class MultiPassPDQNAgent_Threshold(PDQNAgent_Threshold):
    NAME = "Multi-Pass P-DQN Agent with Threshold"
    ...
    def __init__(self,
        ...
        ## 814 update
        self.actor_twin = MultiPassQActor(self.observation_space.shape[0], self.num_actions, self.action_parameter_sizes,
                                     **kwargs['actor_kwargs']).to(device)
        self.actor_twin_target = MultiPassQActor(self.observation_space.shape[0], self.num_actions,
                                            self.action_parameter_sizes,
                                            **kwargs['actor_kwargs']).to(device)
        hard_update_target_network(self.actor_twin, self.actor_twin_target)
        self.actor_twin_target.eval()
        self.actor_twin_optimiser = optim.Adam(self.actor_twin.parameters(), lr=self.learning_rate_actor)
