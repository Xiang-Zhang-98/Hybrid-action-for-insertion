...
class Peginhole_env(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        ...
        mujoco_env.MujocoEnv.__init__(self,
                                      os.getcwd()+'/insertion/peginhole_env/peginhole_env/envs/fall2020_peginhole_square_ori.xml', 1)
        #                               1)
    ...
    def step(self,action):
        ...
        for i in range(100):
            ob = self._get_obs()
            curr_force = ob[12:15]
            if np.abs(np.dot(curr_force,action[0:3])/np.linalg.norm(action[0:3],ord=2)) >force_limit:
                break
            delta_ob = ob - init_ob
